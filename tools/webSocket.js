...
function carNumF(id) {
    carsList = JSON.parse(localStorage.getItem('carList'))
    for (id in carsList) {
        if (id in carsList[id]) {
            console.log(carList[id].number);
            return carList[num].number;
        }
    }
}
    ...
    if (ws?.event == "changes") {
        ...
        for (id in items) {
            ...
            if (position != undefined) {
                ...
                addMarker([lat, long], carNum = String(items[id].id))
